...
const spendBaseFields = {
    ...
    inputUnit:AvCurrencyField({required:true,label:"Input Unit",description:`The currency of the amount, can be one of [${supportedCurrencies.join(' | ')}]`}),
    date: AvDateField({required:true,label:'Date'}),
    emissionCategory:
        AvEmissionCategoryTextField(3)({required:false,label:"GHG Emission Category",}),
    country:AvCountryField({countries,errorMessage:'Could not validate this country. Please check the guide for a list of valid country names.'})({required:false,label:"Country",}),
} as {[index in SpendEntryFlatfileRecordKeys]:AnyField};



export const AvSpendWithSubOrgSheet = new Sheet('Spend With SubOrg Sheet', {
    ...spendBaseFields,
